...
pub mod closures {
    ...
    //      by value: T
    // They preferentially capture variables by reference
    // and only go lower when required.
    ...
    pub fn capturingexample() {
        ...
        // because it is no longer used in the code below.
    ...
    // the trait bound of F specifies the
    fn applytest<F>(f: F) -> i32
    where
        F: FnOnce(i32) -> i32,
    {
        3
    ...
    fn create_fn_para<'a>(text: String, t2: String) -> impl Fn() + 'a {
        move || {
